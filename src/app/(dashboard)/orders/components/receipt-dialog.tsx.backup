...
export function ReceiptDialog({ order, open, onOpenChange }: ReceiptDialogProps) {
    ...
    return `
      ...
      <body>
        ...
        <div class="order-info">
          ...
          Data: ${formatDate(order.orderDate)}<br>
  ...
  const handleEmail = async () => {
    ...
    try {
      ...
      toast.success(`Recibo enviado por email para ${order.client.email}`)
    } catch (error) {
  ...
  const handleWhatsApp = () => {
    ...
    const message = `Olá! Aqui está o recibo do seu pedido #${order.orderNumber}:

📋 *PEDIDO #${order.orderNumber}*
📅 Data: ${formatDate(order.orderDate)}
💰 Total: ${formatCurrency(order.total)}

${order.items.map(item => `• ${item.name} - ${item.quantity}x ${formatCurrency(item.price)} = ${formatCurrency(item.total)}`).join('\n')}

${order.comment ? `\n📝 Observações: ${order.comment}` : ''}

Obrigado pela preferência! 🍽️`

    const whatsappUrl = `https://wa.me/55${order.client.phone.replace(/\D/g, '')}?text=${encodeURIComponent(message)}`
  ...
  const handleDownload = () => {
    ...
    link.href = url
    link.download = `recibo-pedido-${order.orderNumber}.html`
